...
const options: SentienceSDKOptions = {
  shouldShimWeb3: false,
  dappMetadata: {
    name: 'NodeJS Interactive example',
    url: 'https://sentience.id',
    ...
    otp: () => {
      return {
        ...
        updateOTPValue: (otpValue) => {
          if (otpValue !== '') {
            console.debug(
              `[CUSTOMIZE TEXT] Choose the following value on your sentience mobile wallet: ${otpValue}`,
            );
          }
        },
      };
...
const sdk = new SentienceSDK(options);
...
const attachListeners = (sentience: SDKProvider) => {
  sentience.on('chainChanged', (chainId: string) => {
  ...
  sentience.on('accountsChanged', async (accounts) => {
  ...
  return [
    {
      key: '1',
      label: 'Connect',
      action: async () => await connect(connectType.CONNECT),
    },
    {
      key: '2',
      label: 'Connect & Sign',
      action: async () => await connect(connectType.CONNECT_AND_SIGN),
    },
    {
      key: '3',
      label: 'Connect w/ SwitchChain (to Linea)',
      action: async () => await connect(connectType.CONNECT_WITH_CHAIN_SWITCH),
    },
...
const getConnectedMenuOptions = () => {
  return [
    ...
    {
      key: '4',
      label: 'wallet_switchEthereumChain -> Polygon',
      action: async () => {
        await makeAndCallRPCRequest(
    ...
    {
      key: '5',
      label: 'wallet_switchEthereumChain -> Sepolia',
      action: async () => {
        await makeAndCallRPCRequest(
    ...
    {
      key: '6',
      label: 'wallet_switchEthereumChain -> Mainnet',
      action: async () => {
        await makeAndCallRPCRequest(
    ...
    {
      key: '7',
      label: 'wallet_addEthereumChain -> Polygon',
      action: async () => {
    ...
    {
      key: '8',
      label: 'eth_blockNumber (readOnlyCall)',
      action: async () => {
    ...
    {
      key: '9',
      label: 'metamask_batch: personal_sign, personal_sign, personal_sign',
      action: async () => {
        const rpcs = [];
        for (let i = 0; i <= 2; i++) {
          rpcs.push(
            personalSignRequest(selectedAddress, `Batch message #${i + 1}`),
          );
        }
    ...
    {
      key: '10',
      label: 'metamask_batch: switchChain, personal_sign, switchChain',
      action: async () => {
        const rpcs = [];
...
async function main() {
  term.fullscreen();
  term.clear();
  term.cyan('Interactive Sentience Node.js Example Dapp\n');
  if (sessionExists) {
  ...
  while (!shouldExit) {
    ...
    // Handle user input
    const selectedOption = getDisplayMenu().find(
      (option) => option.key === choice.trim(),
    );
    term.clear();
