...
export const DAPPView = ({ sdk }: DAPPViewProps) => {
  ...
  useEffect(() => {
    ...
    try {
      console.debug(
        `useffect sentience.selectedAddress=${sdk
          .getProvider()
          ?.getSelectedAddress()}`,
      );
      ...
      sdk.getProvider()?.on('chainChanged', (newChain: unknown) => {
        console.log('useEffect::sentience on "chainChanged"', newChain);
        setChain(newChain as string);
      });

      sdk.getProvider()?.on('_initialized', () => {
        console.log(
          `useEffect::sentience on "_initialized" sentience.selectedAddress=${sdk
            .getProvider()
            ?.getSelectedAddress()} sentience.chainId=${sdk.getProvider()?.getChainId()}`,
        );
        if (sdk.getProvider()?.getSelectedAddress()) {
          setAccount(sdk.getProvider()?.getSelectedAddress() as string);
        }
      ...
      sdk.getProvider()?.on('accountsChanged', (_accounts: unknown) => {
        ...
        console.log('useEffect::sentience on "accountsChanged"', accounts);
      ...
      sdk.getProvider()?.on('disconnect', () => {
        console.log('useEffect::sentience on "disconnect"');
        setConnected(false);
      });
