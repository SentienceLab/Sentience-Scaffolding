...
import { ServiceStatus } from '@sntns/sdk';
...
export const DAPPView = (_props: DAPPViewProps) => {
  ...
  const exampleRequest = async () => {
    try {
      setResponse('');
      const result = await sentience?.request({
  ...
  const testEthers = async () => {
    ...
    const web3Provider = new ethers.providers.Web3Provider(
      sentience as unknown as ethers.providers.ExternalProvider,
    );
  ...
  const sign = async () => {
    const msgParams = JSON.stringify({
      domain: {
        ...
        chainId: sentience?.getChainId()
          ? parseInt(sentience.getChainId(), 16)
          : 1,
    ...
    const from = sentience?.getSelectedAddress();

    const params = [from, msgParams];
    const method = 'eth_signTypedData_v4';

    setResponse('');
    const resp = await sentience?.request({ method, params });
  ...
  const sendTransaction = async () => {
    ...
    const transactionParameters = {
      ...
      from: sentience?.getSelectedAddress(), // must match user's active address.
    ...
    try {
      ...
      const txHash = await sentience?.request({
  ...
  const testReadOnlyCalls = async () => {
    try {
      const chain = await sentience?.request({
        method: 'eth_chainId',
        params: [],
      });
      ...
      const b1 = await sentience?.request({
        method: 'eth_getBalance',
        params: [acc1, 'latest'],
      });
      const acc2 = '0xA9FBbc6C2E49643F8B58Efc63ED0c1f4937A171E';
      const b2 = await sentience?.request({
        method: 'eth_getBalance',
        params: [acc2, 'latest'],
      });
  ...
  const batch = async () => {
    const selectedAddress = sentience?.getSelectedAddress();
    ...
    try {
      setResponse('');
      const response = (await sentience?.request({
