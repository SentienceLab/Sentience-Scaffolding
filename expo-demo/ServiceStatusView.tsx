...
import type { ServiceStatus } from '@sntns/sdk';
...
export const ServiceStatusView = ({
  serviceStatus,
  ...
  return (
    <View>
      ...
      <Text style={{ color: 'black' }}>
        Channel: {serviceStatus?.channelId}
      </Text>
      <Text
        style={{ color: 'black' }}
      >{`Expiration: ${serviceStatus?.channelConfig?.validUntil ?? ''}`}</Text>
    </View>
  );
};
